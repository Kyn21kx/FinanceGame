...
func render_property(comp_instance, prop_name: String, type: int, is_read_only: bool = false) -> Control:
	...
	match type:
		TYPE_BOOL:
			input_control = UIUtils._create_bool_input(current_value, is_read_only)
		
		TYPE_INT:
			input_control = UIUtils._create_int_input(current_value, is_read_only)
		
		TYPE_FLOAT:
			input_control = UIUtils._create_float_input(current_value, is_read_only)
		
		TYPE_STRING:
			input_control = UIUtils._create_string_input(current_value, is_read_only)
		
		TYPE_VECTOR2:
			input_control = UIUtils._create_vector2_input(current_value, is_read_only)
		
		TYPE_VECTOR3:
			input_control = UIUtils._create_vector3_input(current_value, is_read_only)
		
		TYPE_VECTOR4:
			input_control = UIUtils._create_vector4_input(current_value, is_read_only)
		
		_:
			# Fallback for unsupported types
			input_control = UIUtils._create_string_input(str(current_value), is_read_only)
